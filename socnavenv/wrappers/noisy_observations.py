...
class NoisyObservations(gym.Wrapper):
    ...
    def observation_space(self):
        ...
        if self.env.is_entity_present["humans"]:
            d["humans"] =  spaces.Box(
                low=np.array([0, 0, 0, 0, 0, 0, -self.MAP_X * np.sqrt(2)-self.max_noise, -self.MAP_Y * np.sqrt(2)-self.max_noise, -1.0-self.max_noise, -1.0-self.max_noise, -self.HUMAN_DIAMETER/2-self.max_noise, -(self.MAX_ADVANCE_HUMAN + self.MAX_ADVANCE_ROBOT)*np.sqrt(2)-self.max_noise, -2*np.pi/self.TIMESTEP -self.max_noise, 0-self.max_noise] * ((self.MAX_HUMANS + self.MAX_H_L_INTERACTIONS + (self.MAX_H_H_DYNAMIC_INTERACTIONS*self.MAX_HUMAN_IN_H_H_INTERACTIONS) + (self.MAX_H_H_STATIC_INTERACTIONS*self.MAX_HUMAN_IN_H_H_INTERACTIONS)) if self.get_padded_observations else self.total_humans), dtype=np.float32),
                high=np.array([1, 1, 1, 1, 1, 1, +self.MAP_X * np.sqrt(2)+self.max_noise, +self.MAP_Y * np.sqrt(2)+self.max_noise, 1.0+self.max_noise, 1.0+self.max_noise, self.HUMAN_DIAMETER/2+self.max_noise, +(self.MAX_ADVANCE_HUMAN + self.MAX_ADVANCE_ROBOT)*np.sqrt(2)+self.max_noise, +2*np.pi/self.TIMESTEP +self.max_noise, 1+self.max_noise] * ((self.MAX_HUMANS + self.MAX_H_L_INTERACTIONS + (self.MAX_H_H_DYNAMIC_INTERACTIONS*self.MAX_HUMAN_IN_H_H_INTERACTIONS) + (self.MAX_H_H_STATIC_INTERACTIONS*self.MAX_HUMAN_IN_H_H_INTERACTIONS)) if self.get_padded_observations else self.total_humans), dtype=np.float32),
        ...
        if self.env.is_entity_present["laptops"]:
            d["laptops"] =  spaces.Box(
                low=np.array([0, 0, 0, 0, 0, 0, -self.MAP_X * np.sqrt(2)-self.max_noise, -self.MAP_Y * np.sqrt(2)-self.max_noise, -1.0-self.max_noise, -1.0-self.max_noise, -self.LAPTOP_RADIUS-self.max_noise, -(self.MAX_ADVANCE_ROBOT)*np.sqrt(2)-self.max_noise, -self.MAX_ROTATION-self.max_noise, 0-self.max_noise] * ((self.MAX_LAPTOPS + self.MAX_H_L_INTERACTIONS) if self.get_padded_observations else (self.NUMBER_OF_LAPTOPS + self.NUMBER_OF_H_L_INTERACTIONS)), dtype=np.float32),
                high=np.array([1, 1, 1, 1, 1, 1, +self.MAP_X * np.sqrt(2)+self.max_noise, +self.MAP_Y * np.sqrt(2)+self.max_noise, 1.0+self.max_noise, 1.0+self.max_noise, self.LAPTOP_RADIUS+self.max_noise, +(self.MAX_ADVANCE_ROBOT)*np.sqrt(2)+self.max_noise, +self.MAX_ROTATION+self.max_noise, 1+self.max_noise] * ((self.MAX_LAPTOPS + self.MAX_H_L_INTERACTIONS) if self.get_padded_observations else (self.NUMBER_OF_LAPTOPS + self.NUMBER_OF_H_L_INTERACTIONS)), dtype=np.float32),
